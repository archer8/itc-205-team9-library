...
public class Main implements IMainListener {
	...
	private void setupTestData() {
		...
		for(int checkDate = 0; checkDate < 2; ++checkDate) {
			ILoan i = this.loanDAO.createLoan(member[1], book[checkDate]);
			this.loanDAO.commitLoan(i);
		}

		cal.setTime(now);
		cal.add(5, 15);
		Date var8 = cal.getTime();
		this.loanDAO.updateOverDueStatus(var8);
		member[2].addFine(10.0F);

		ILoan loan;
		int var9;
		for(var9 = 2; var9 < 7; ++var9) {
			loan = this.loanDAO.createLoan(member[3], book[var9]);
			this.loanDAO.commitLoan(loan);
		}

		member[4].addFine(5.0F);

		for(var9 = 7; var9 < 9; ++var9) {
			loan = this.loanDAO.createLoan(member[5], book[var9]);
			this.loanDAO.commitLoan(loan);
		}
